...
class GoogleNews:
    ...
    def __parse_feed(self, feed_url, proxies=None, scraping_bee = None):
        ...
        #MODIFICATO DA ME
        if d['entries']:
            first_article = d['entries'][0]  # Prende solo il primo articolo
            return {'feed': d['feed'], 'entries': [first_article]}  # Restituisce un dizionario con il feed e il primo articolo
        else:
            # Se non ci sono articoli, restituisce il feed con una lista di entries vuota
            return {'feed': d['feed'], 'entries': []}
    ...
    def topic_headlines(self, topic: str, proxies=None, scraping_bee=None):
        ...
        #d['entries'] = self.__add_sub_articles(d['entries'])  #MODIFICATO DA ME, non mi servono i subarticoli
        if len(d['entries']) > 0:
            return d
        else:
